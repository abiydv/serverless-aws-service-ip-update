...
def initial_handler(event, context):
    print("Received event: " + json.dumps(event, indent=2))
    message = json.loads(event['Records'][0]['Sns']['Message'])
    sgregion = os.environ['SGREGION']
    ...
    result = update_security_groups(ip_ranges,sgregion)
    return result
...
def update_security_groups(new_ranges,sgregion):
    client = boto3.client('ec2', region_name=sgregion)
